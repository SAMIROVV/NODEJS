...
for (i in urls) {
    ...
    res.on('end', () => {
		   ...
		   if(type === 'https://trends.google.com/trends/api/autocomplete/'){ var clean = JSON.parse(data.slice(5)).default.topics.map(function (item) {  return item.title+'☔☉'+item.type}).join('💙💔');
	           ...
	           if(type === 'https://trends.google.com/trends/api/realtimetrends?'){ var clean  = JSON.parse(data.slice(5)).storySummaries.trendingStories.map(function (item) {  return item.articles.map(function (item) {  return item.articleTitle+'☔☉'+item.source+'☔☉'+item.time.replace('il y a','')+'☔☉'+item.snippet}).join('💙💔')})       		
										      responses.push(urls[i].split('🔸')[1]+'💚'+unescapeHTML(clean));
	      }   
