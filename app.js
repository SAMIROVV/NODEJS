...
const options = {
  ...
  directory: decodeURIComponent(url.format({protocol: req0.protocol,   host: req0.get('host'),    pathname: req0.originalUrl}))
   ...
   console.log(decodeURIComponent(url.format({ pathname: req0.originalUrl })).split("☆")[0].replace('/SCRAPER6?',''));console.log('directory', decodeURIComponent(url.format({protocol: req0.protocol,   host: req0.get('host'),    pathname: req0.originalUrl})));
		res0.header('Content-Type', 'application/json').send(body).end();  
        });});
